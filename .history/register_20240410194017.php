...
if (isset($_POST['username'], $_POST['password'], $_POST['confirm_password'],  $_POST['email'])) {
	...
	// $stmt->execute();
	// Username doesn't exists, insert new account
$stmt = $conn->prepare('INSERT INTO accounts (username, password, email, activation_code) VALUES (?, ?, ?, ?)')) {	// We do not want to expose passwords in our database, so hash the password and use password_verify when a user logs in.
	$password = password_hash($_POST['password'], PASSWORD_DEFAULT);
	$uniqid = uniqid();
...
// }
	...
	try {
		...
		$mail->addAddress($_POST['email'], $_POST[]); // Adresse email du destinataire et nom du destinataire

		// Contenu du message
		$mail->isHTML(true);
		$mail->Subject = 'Account Activation Required';
		$headers = 'From: ' . $from . "\r\n" . 'Reply-To: ' . $from . "\r\n" . 'PHPMailer: PHP/' . phpversion() . "\r\n" . 'MIME-Version: 1.0' . "\r\n" . 'Content-Type: text/html; charset=UTF-8' . "\r\n";
		$activate_link = 'http://thedistrict@mail.com/phplogin/activate.php?email=' . $_POST['email'] . '&code=' . $uniqid . $activation_code;
